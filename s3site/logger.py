...
def get_s3site_logger():
    log = logging.getLogger('s3site')
    log.addHandler(NullHandler())
    return log


log = get_s3site_logger()
console = ConsoleLogger()
session = logging.StreamHandler(StringIO.StringIO())


def configure_s3site_logging(use_syslog=False):
    """
    Configure logging for s3site *application* code

    By default s3site's logger has no formatters and a NullHandler so that
    other developers using s3site as a library can configure logging as
    they see fit. This method is used in s3site's application code (i.e.
    the 's3site' command) to toggle s3site's application specific
    formatters/handlers
